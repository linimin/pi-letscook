...
- removed the remaining validated `recent_discussion` startup-analysis fallback from `/cook`, so workflow entry now succeeds only when same-entry primary-agent handoff synthesis produces a startable startup brief and otherwise fails closed
