...
- read current repo truth plus tracked `.cook/**` policy and ignored `.agent/current/**` state when it already exists
- create or repair tracked completion contract files under `.cook/**`
- update `.gitignore` so tracked `.cook/**` files remain tracked while `.agent/**` execution artifacts remain ignored
...
- edit tracked product, docs, config, or test files outside `.cook/**`, local `.agent/**` helper paths, and `.gitignore`
...
4. Create or repair `.cook/README.md`, `.cook/workflow.json`, and `.cook/profile.json`, and create or repair local `.agent/verify_completion_stop.sh` / `.agent/verify_completion_control_plane.sh` forwarders when they are needed for truthful local verification.
5. Update `.gitignore` so `.agent/**` remains ignored and `.cook/**` remains tracked, while keeping `.agent/current/tmp/` ignored as scratch space.
