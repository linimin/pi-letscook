...
assertIncludes('extensions/completion/role-runner.ts', 'systemPromptTemp ??= await writeTempFile(params.root, "pi-completion-role-", agent.systemPrompt);');
...
assertIncludes('extensions/completion/role-runner.ts', 'backend === "cursor-sdk"');
assertIncludes('extensions/completion/role-runner.ts', 'backend === "cursor-cli-ask"');
assertIncludes('extensions/completion/role-runner.ts', 'subprocessRunner ?? defaultPiRoleRunner');
assertIncludes('extensions/completion/role-runner.ts', 'const combinedPrompt = `${agent.systemPrompt}\\n\\n${prompt}`;');
