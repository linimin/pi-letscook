...
assertIncludes('docs/PROTOCOL.md', 'canonical implementation contract for selected, in-progress, committed, and done slices');
assertIncludes('docs/PROTOCOL.md', 'The selected plan slice must mirror that exact contract across goal, contract IDs, acceptance criteria');
assertIncludes('docs/PROTOCOL.md', '`basis_commit`');
assertIncludes('docs/PROTOCOL.md', '`remaining_contract_ids_before` plus `release_blocker_count_before` / `high_value_gap_count_before`');
assertIncludes('docs/PROTOCOL.md', 'Deterministic active-slice contract regression now lives in `bash scripts/active-slice-contract-test.sh`');
assertIncludes('docs/PROTOCOL.md', 'includes deterministic active-slice contract coverage plus observability coverage');
