...
assertIncludes('agents/completion-bootstrapper.md', '.agent/current/tmp/');
assertExcludes('agents/completion-bootstrapper.md', '.agent/state.json');
assertExcludes('agents/completion-bootstrapper.md', '.agent/plan.json');
assertExcludes('agents/completion-bootstrapper.md', '.agent/active-slice.json');
assertExcludes('agents/completion-bootstrapper.md', '.agent/tmp/');
assertExcludes('agents/completion-bootstrapper.md', '.agent/profile.json');
assertIncludes('agents/completion-stop-judge.md', '.agent/current/stop-check-history.jsonl');
assertIncludes('agents/completion-stop-judge.md', '.agent/current/state.json current_stop_wave_id');
assertIncludes('agents/completion-stop-judge.md', '.agent/current/plan.json');
assertExcludes('agents/completion-stop-judge.md', '.agent/stop-check-history.jsonl');
assertExcludes('agents/completion-stop-judge.md', '.agent/state.json current_stop_wave_id');
assertExcludes('agents/completion-stop-judge.md', '.agent/plan.json');
assertIncludes('docs/maintainer/protocol.md', 'canonical implementation contract for selected, in-progress, committed, and done slices');
assertIncludes('docs/maintainer/protocol.md', 'The selected plan slice must mirror that exact contract across goal, contract IDs, acceptance criteria');
assertIncludes('docs/maintainer/protocol.md', '`basis_commit`');
assertIncludes('docs/maintainer/protocol.md', '`remaining_contract_ids_before` plus `release_blocker_count_before` / `high_value_gap_count_before`');
assertIncludes('docs/maintainer/protocol.md', 'Deterministic active-slice contract regression now lives in `bash scripts/active-slice-contract-test.sh`');
assertIncludes('docs/maintainer/protocol.md', 'includes deterministic active-slice contract coverage plus observability coverage');
